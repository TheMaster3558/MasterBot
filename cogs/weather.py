...
class Weather(Cog, name="weather"):
    ...
    async def fetch_units(self):
        for guild in self.bot.guilds:
            async with self.db.execute(
                f"""SELECT temp, speed FROM units
                                        WHERE id = {guild.id};"""
            ) as cursor:
        ...
        for guild in self.bot.guilds:
            ...
            try:
                await self.db.execute(
                    f"""INSERT INTO units VALUES ({guild.id},
    ...
    async def units(self, ctx, *, flags: FlagUnits | str = None):
        ...
        await ctx.send(
            f"New settings! Temp: `{self.temp_units[ctx.guild.id]}` Speed: `{self.speed_units[ctx.guild.id]}`"
        )
    ...
    async def _units(
        ...
        temp: Literal["C", "F"] = None,
        speed: Literal["kph", "mph"] = None,
    ):
        ...
        if not temp and not speed:
            await interaction.response.send_message(
                "You must give at least one argument."
            )
            return
