...
class FBIHTTPClient(AsyncHTTPClient):
    ...
    async def general_wanted(
        self, *, title: str = None, person: str = None, status: str = None
    ):
...
class StandardSelect(discord.ui.Select["FBIView"]):
    def __init__(self, option: Literal["types", "statuses"]):
        options = [discord.SelectOption(label="Any")] + [
            discord.SelectOption(label=label)
            for label in getattr(FBIHTTPClient, option)
        ]
    ...
    async def callback(self, interaction: discord.Interaction):
        if len(self.values) > 1 and "Any" in self.values:
            await interaction.response.send_message(
                "You can't select `Any` and other options.", ephemeral=True
            )
            self.failed = True
...
class FBIWanted(Cog):
    ...
    async def wanted(self, ctx, title: str):
        ...
        if status == "Any":
            status = None
        data = await self.http.general_wanted(title=title, person=person, status=status)
        print(data)
