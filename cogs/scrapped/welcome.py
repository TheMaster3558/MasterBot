...
class Help:
    ...
    def welcome_set(self):
        message = (
            f"`{self.prefix}welcome set join <message>`: Set the message for when users join\n"
    ...
    def welcome_remove(self):
        message = (
            f"`{self.prefix}welcome remove join`: Remove the join message\n"
...
def get_join_message(member):
    ...
    if messages.get(str(member.guild.id)):
        ...
        return None
    return discord.Embed(
        title=f"Welcome to **{member.guild.name}**", description="We hope you have fun!"
    )
...
class WelcomeMessages(commands.Cog):
        ...
        if channel is not None:
            if channel.permissions_for(member.guild.me).send_messages:
                ...
                if message is not None:
                    await channel.send(member.mention, embed=message)
    ...
    async def set(self, ctx):
        if ctx.invoked_subcommand is None:
            embed = discord.Embed(
                title="Options", description="`join`\n`leave`\n`channel`"
            )
