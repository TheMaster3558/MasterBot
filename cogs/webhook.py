...
class WebhookGroup(app_commands.Group):
    def __init__(self, cog: Webhooks):
        self.cog = cog
        super().__init__(
            name="webhook", description="A group command to make your own webhook!"
        )
    ...
    async def create(
        self, interaction: discord.Interaction, name: str, avatar: str = None
    ):
        if self.cog.users.get(interaction.user.id):
            view = ConfirmView()
            await interaction.response.send_message(
                "Are you sure you would like to replace your old webhook users?",
                view=view,
            )
            await view.wait()
        ...
        if avatar is not None:
            if not avatar.startswith("http://") and not avatar.startswith("https://"):
                await smart_send(
                    interaction,
                    "Avatar must be a url starting with `http://` or `https://`",
                )
                return
    ...
    async def send(self, interaction: discord.Interaction, content: str):
        ...
        await webhook.send(
            content=content, username=data["name"], avatar_url=data["avatar"]
        )
        await interaction.response.send_message(
...
class Webhooks(Cog, name="webhooks"):
    ...
    async def cog_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.BotMissingPermissions):
            if "manage_webhooks" in error.missing_permissions:
                embed = discord.Embed(
                    title="I need `manage_webhooks` permissions.",
    ...
    async def fetch_webhooks(self):
        for channel in self.bot.get_all_channels():
            async with self.db.execute(
                f"""SELECT webhook_id, webhook_token FROM webhooks
                                           WHERE id = {channel.id};"""
            ) as cursor:
        ...
        for user in users:
            cursor = await self.db.execute(
                f"""SELECT name, avatar FROM users
                                            WHERE id = {user.id}"""
            )
            data = await cursor.fetchone()
            if data is None:
        ...
        for k, v in self.webhooks.items():
            try:
                await self.db.execute(
                    f"""INSERT INTO webhooks VALUES ({k},
                {v.id},
                '{v.token}')"""
                )
    ...
    async def webhook(self, ctx: commands.Context):
        if ctx.invoked_subcommand is None:
            embed = discord.Embed(title="Categories", description="`create`\n`send`")
    ...
    async def create(self, ctx: commands.Context, *, flags: WebhookUserFlags):
        ...
        if flags.avatar is not None:
            if not flags.avatar.startswith("http://") and not flags.avatar.startswith(
                "https://"
            ):
    ...
    async def error(self, ctx, error):
        if isinstance(
            ...
            embed.set_footer(
                text=f"Example: {ctx.prefix}create name: XbowMaster avatar: https://someurl.com/"
            )
            await ctx.send(embed=embed)
        else:
            raise error
